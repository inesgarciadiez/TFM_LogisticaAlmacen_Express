...
const { getAllByEstadosYUsuario, updateState, getAllClosedStateAndUser, getAllPedidosByEncargado } = require("../../models/pedido.model");
const { checkToken, checkOperario } = require("../../utils/middlewares.js");
...
router.get('/encargado', checkEncargado, async (req, res) => {
  const usuarioId = req.user.id;
  try {
    const [result] = await getAllByEstadosYUsuario(estadosEncargado, usuarioId);
    res.json(result);
    console.log(result);
  } catch (error) {
